...
export function BatchVideoUpload() {
  return <BatchGenerationFlow generationType="video" />;
}

// Legacy component - keeping for backward compatibility
export function BatchVideoUploadLegacy() {
  ...
  const [generationMode, setGenerationMode] = useState<'t2v' | 'i2v'>('t2v');
  ...
  const handleDownloadTemplate = async (format: 'excel' | 'csv') => {
    try {
      const response = await fetch(
        `/api/v1/workflow/template/download?format=${format}&generationType=video`
      );
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `batch-video-generation-template.${format === 'excel' ? 'xlsx' : 'csv'}`;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      formData.append('generationType', 'video');
  ...
  return (
    <Card className="p-6">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold mb-2">批量视频生成</h2>
          <p className="text-slate-600 dark:text-slate-400">上传Excel/CSV文件，批量生成产品视频</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setGenerationMode(value as 't2v' | 'i2v')}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <div className="border rounded-lg p-4 bg-slate-50 dark:bg-slate-900">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            模板包含字段：prompt（必需）,
            baseImageUrl（可选，用于图生视频，支持URL、base64或Excel中直接插入图片）, model（可选）,
