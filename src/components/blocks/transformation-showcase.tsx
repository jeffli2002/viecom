...
import { Shirt, RefreshCw, Box, Camera, Sparkles, ChevronLeft, ChevronRight, Check, Maximize2 } from 'lucide-react';

type ScenarioId = 'virtualTryOn' | 'modelSwap' | 'furnitureStaging' | 'sceneGeneration' | 'holidayCampaign';
    ...
    <section id="studio" className="section-base bg-main overflow-hidden">
      <div className="container-base relative z-10">
        <div className="text-center mb-16">
           <span className="text-teal-500 font-medium tracking-wider text-sm uppercase mb-2 block">{t('badge')}</span>
            ...
            <div className="grid lg:grid-cols-12 gap-6 items-center max-w-7xl mx-auto min-h-[600px]">
               <div className="lg:col-span-5 space-y-6 relative z-20">
                  <div className="relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-teal-500 to-blue-600 rounded-2xl opacity-20 blur transition duration-500 group-hover:opacity-40"></div>
                    <div className="glass-card p-8 rounded-2xl relative shadow-2xl">
                      <div className="flex gap-4 items-center mb-8">
                         <div className="w-10 h-10 rounded-full bg-teal-500/10 flex items-center justify-center text-teal-500 font-bold text-lg">1</div>
                         <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{t('inputAssets')}</h3>
                      </div>
                      
                      <div key={activeScenario.id} className="grid grid-cols-2 gap-6">
                         <div className="space-y-3">
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t(`scenarios.${activeScenario.id}.input1Label`)}</span>
                         ...
                         <div className="space-y-3">
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t(`scenarios.${activeScenario.id}.input2Label`)}</span>
                            <div className="aspect-[3/4] rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 relative group/input cursor-zoom-in shadow-inner">
                      ...
                      <div className="mt-8 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-white/5 pt-6">
                         ...
                         <span className="flex items-center gap-1"><Maximize2 className="w-3 h-3"/> {t('highResReady')}</span>
                      </div>
                    </div>
                  </div>
               </div>
               ...
               <div className="absolute left-[50%] top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 hidden lg:flex flex-col items-center justify-center pointer-events-none">
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-full shadow-2xl border border-teal-500/20 text-teal-500 relative">
                        ...
                        <div className="absolute -inset-1 bg-teal-500/20 rounded-full blur animate-ping opacity-20"></div>
                    </div>
                  ...
                  <div className="relative w-full max-w-[380px] aspect-[4/5] group/result cursor-pointer">
                      <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border-2 border-teal-500/50 transition-all duration-500 group-hover/result:shadow-teal-500/50 group-hover/result:shadow-2xl group-hover/result:scale-105 group-hover/result:border-teal-500">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-8 pt-32 transition-all duration-500">
                            ...
                            <h3 className="text-white font-bold text-2xl mb-2">{t(`scenarios.${activeScenario.id}.title`)}</h3>
