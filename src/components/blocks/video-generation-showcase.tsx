...
import { Play, Pause, Upload, Sparkles, Loader2, Cpu, Check, Wand2, Camera, Ratio, Film, Clock, Maximize2, Volume2, VolumeX } from 'lucide-react';
...
export function VideoGenerationShowcase() {
  ...
  useEffect(() => {
    ...
    veoDemos.forEach(demo => {
      ...
      const captureTempThumbnail = () => {
        ...
        try {
          ...
          if (!tempVideo.videoWidth || !tempVideo.videoHeight) {
            console.warn(`Video ${demo.id} (${demo.filename || demo.video}) dimensions not available yet`);
            return;
          }
          ...
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              try {
                ...
                setVideoThumbnails(prev => {
                  if (!prev[demo.id]) {
                    console.log(`✅ Thumbnail captured for queue item ${demo.id} (${demo.category}) - ${demo.filename || demo.video}`);
                    captured = true;
                    processingThumbnailsRef.current.delete(demo.id);
                    cleanup();
      ...
      // 开始加载视频
      console.log(`🔄 Loading video for thumbnail: ${demo.filename || demo.video} (ID: ${demo.id})`);
      tempVideo.load();
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
          <div>
            <h2 className="h2-section">
               {t('title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-blue-500">{t('titleHighlight')}</span>
            </h2>
            ...
            <div className="flex flex-col lg:flex-row h-full overflow-hidden">
                    ...
                    <div className="h-14 flex items-center justify-between px-6 border-b border-slate-800 bg-slate-900 z-20 flex-shrink-0">
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 px-3 py-1 rounded bg-red-500/10 border border-red-500/20 backdrop-blur-md">
                                ...
                                <span className="text-[10px] font-bold text-red-500 tracking-widest">{t('recording')}</span>
                            </div>
                        ...
                        {isGenerating && (
                            <div className="absolute inset-0 z-50 bg-[#0f172a]/95 backdrop-blur-md flex flex-col items-center justify-center p-8 transition-all duration-300">
                                <div className="relative w-32 h-32 mb-8">
                                    ...
                                    <div className="absolute inset-4 border-2 border-blue-500 rounded-full border-b-transparent animate-spin opacity-70" style={{animationDirection: 'reverse', animationDuration: '3s'}}></div>
                                ...
                                <div className="w-64 grid grid-cols-4 gap-1">
                                     {[1, 2, 3, 4].map((i) => (
                                         <div key={i} className={`h-1 rounded-full transition-colors duration-300 ${
                                             progress >= i * 25 ? 'bg-teal-500 shadow-[0_0_10px_rgba(20,184,166,0.5)]' : 
                                             progress >= (i-1) * 25 + 10 ? 'bg-teal-500/50' : 'bg-slate-800'
                                         }`}></div>
                                     ))}
                                </div>
                                ...
                                <div className="mt-8 font-mono text-[10px] text-slate-500 space-y-1 text-center opacity-70">
                                    ...
                                    <div className={progress > 20 ? 'text-slate-400' : 'hidden'}> &gt; {t('geometryInference')}</div>
                                    <div className={progress > 50 ? 'text-slate-400' : 'hidden'}> &gt; {t('physicsSimulation')}</div>
                                </div>
                            </div>
                        )}
                        ...
                        <div 
                            ...
                            onClick={(e) => {
                                if (videoRef.current && videoDuration > 0) {
                                    const progressBar = e.currentTarget.querySelector('.progress-bar-container') as HTMLElement;
                                    if (progressBar) {
                ...
                <div className="w-full lg:w-80 lg:flex-shrink-0 bg-slate-900 border-t lg:border-t-0 lg:border-l border-slate-800 flex flex-col h-full">
                     <div className="p-4 border-b border-slate-800 flex items-center justify-between flex-shrink-0">
                        ...
                        <span className="text-[10px] text-slate-600">{veoDemos.length} {t('items')}</span>
                     </div>
