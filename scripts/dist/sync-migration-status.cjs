...
// Load .env.local file
(0, dotenv_1.config)({ path: (0, node_path_1.resolve)(process.cwd(), '.env.local') });
...
async function syncMigrationStatus() {
    ...
    `);
        ...
        const hasRewardTables = existingTables.includes('user_daily_checkin') || existingTables.includes('user_referral');
    ...
    `);
        ...
        console.log('\n🔍 Migration status:');
        console.log(`   Migration 0000 (account/user tables): ${hasAccountTable && hasUserTable ? '✅ Applied' : '❌ Not applied'}`);
        console.log(`   Migration 0001 (reward tables): ${hasRewardTables ? '✅ Applied' : '❌ Not applied'}`);
        console.log(`   Migration 0003 (scheduled columns): ${hasScheduledColumns ? '✅ Applied' : '❌ Not applied'}`);
        console.log(`   Migration 0004 (purchase source): ${hasPurchaseSource ? '✅ Applied' : '❌ Not applied'}`);
        ...
        for (const entry of entries) {
            ...
            try {
                ...
                console.warn(`   ⚠️  Could not process ${tag}:`, error instanceof Error ? error.message : String(error));
            }
        }
