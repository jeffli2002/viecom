...
import { generatedAsset, user, session } from '../src/server/db/schema';
...
async function checkUserIPs() {
  try {
    ...
    const userIPs: Map<string, Array<{ ipAddress: string | null; createdAt: Date; taskId: string }>> = new Map();
    ...
    for (const asset of foundAssets) {
      ...
      if (sessions.length > 0) {
        ...
        if (recentSessions.length > 0) {
          console.log(`   📍 Found ${recentSessions.length} recent session(s) (not in time window):`);
    ...
    const allIPs: Array<{ userId: string; userEmail: string | null; ipAddress: string; taskId: string }> = [];
    ...
    if (allIPs.length === 0) {
      ...
      const uniqueIPs = Array.from(ipGroups.keys());
      const user1IPs = allIPs.filter(ip => ip.userId === foundAssets[0]?.userId).map(ip => ip.ipAddress);
      const user2IPs = allIPs.filter(ip => ip.userId === foundAssets[1]?.userId).map(ip => ip.ipAddress);
