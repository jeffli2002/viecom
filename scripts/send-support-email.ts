...
async function sendSupportEmail() {
  ...
  if (args.length < 3) {
    ...
    console.log(
      '  pnpm tsx scripts/send-support-email.ts user@example.com "Re: Your issue" "Thank you for contacting us..."'
    );
    console.log(
      '  pnpm tsx scripts/send-support-email.ts promodkc@gmail.com "Update on your request" "Your video is ready!"'
    );
    console.log('');
    process.exit(1);
  }
  ...
  try {
    ...
    const messageParagraphs = message
      ...
      .map(
        (p) =>
          `<p style="margin: 0 0 15px 0; color: #374151; line-height: 1.6;">${p.replace(/\n/g, '<br>')}</p>`
      )
      .join('');
